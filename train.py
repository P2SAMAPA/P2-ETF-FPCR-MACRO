...
from fpcr_macro import fpcr_aggregate_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    if macro_df is None or macro_df.empty:
        return None
    macro_window = macro_df.loc[ret_window.index]  # not used in simplified version
    raw_scores = {}
    for ticker in ret_window.columns:
        s = fpcr_aggregate_score(ret_window[ticker].values, macro_window,
                                 n_basis=config.N_BASIS, n_components=config.N_COMPONENTS)
        if not np.isfinite(s):
            s = 0.0
        raw_scores[ticker] = float(s)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "fpcr_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/fpcr_macro_{timestamp}.json"
